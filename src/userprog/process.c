...
int load_arguments_to_stack(int argc, char *argv[], int argv_lengths[], void **if_esp) {
  ...
  uint32_t current_sp = (uint32_t)*if_esp; 

  for (int i=0; i<argc; i++) { // iterate through all argv
    int word_length = argv_lengths[i];
    for (int j=word_length - 1; j >= 0; j--) { // push strings in reverse order
      current_sp--;
      char c = argv[i][j];
      memset(current_sp, c, 1); 
    }
    saved_addresses[i] = current_sp;
  }

  // push stack alignment
  ...
  for (int i = argc-1; i>=0; i++) {
    current_sp -= 4;
    memset(current_sp, saved_addresses[i], 4);
  }
...
uint8_t stack_alignment_calc(uint8_t stack_pointer, int argc) {
  // returns the number of bytes needed to stack align
  
  // we subtract 16 because of argv null, argv, argc, garbage return
  uint32_t end_sp= stack_pointer - 16 - argc * 4;
  uint32_t garbage_bytes = 0;
  while ((end_sp - garbage_bytes) % 16 != 12) {
    garbage_bytes++;
  }
